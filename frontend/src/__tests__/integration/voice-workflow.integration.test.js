/**
 * voice-workflow.integration.test.js - Voice Assistant Workflow Integration Tests
 *
...
describe('Voice Assistant Workflow Integration Tests', () => {
  ...
  describe('Voice Session Workflow', () => {
    test('should start voice session successfully', async () => {
      const mockNestedWS = global.createMockWebSocketServer(
        'ws://localhost:8000/api/runs/MainConversation'
      );
      const mockClaudeWS = global.createMockWebSocketServer(
  ...
  describe('Conversation Management Workflow', () => {
    ...
    test('should create new conversation', async () => {
      server.use(
        rest.post(`${API_URL}/api/realtime/conversations`, (req, res, ctx) => {
          return res(
            ctx.status(201),
            ctx.json({
              id: 'new-conv-id',
    ...
    test('should filter conversations by source', async () => {
      ...
      // Apply filter
      await waitFor(() => {
        const filterSelect = screen.getByLabelText(/filter.*source/i);
