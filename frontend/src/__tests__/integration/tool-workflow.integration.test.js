/**
 * tool-workflow.integration.test.js - Tool Management Workflow Integration Tests
 *
...
describe('Tool Workflow Integration Tests', () => {
  ...
  describe('Tool Upload Workflow', () => {
    ...
    test('should upload tool file successfully', async () => {
      server.use(
        rest.post(`${API_URL}/api/tools/upload`, (req, res, ctx) => {
          return res(
            ctx.status(200),
  ...
  describe('AI Tool Generation Workflow', () => {
    ...
    pass`;
      ...
      // Enter prompt
      await waitFor(() => {
        const promptInput = screen.getByLabelText(/describe.*tool/i);
