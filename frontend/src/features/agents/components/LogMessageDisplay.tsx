...
function parseReprContent(contentStr: string): ParsedFields {
...
const safeParse = (str: unknown): unknown => {
...
// Commented out as unused, but kept for potential future use
// const processMessageContent = (content: unknown): unknown => {
//   if (!content) return '';
...
// Icon Mapping
const getLogTypeIcon = (type?: string): ReactNode => {
  const lowerType = type?.toLowerCase();
  switch (lowerType) {
    case 'error':
    case 'system_error':
...
// ============================================================================
// TextMessage Renderer Component
// ============================================================================

const TextMessageRenderer: FC<{ fields: ParsedFields }> = ({ fields }): JSX.Element => {
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
        <>
          {fields.models_usage && typeof fields.models_usage === 'object' && (
          ...
          <Collapse in={tokensVisible}>
            {fields.models_usage && typeof fields.models_usage === 'object' && fields.models_usage !== null && (
              <Stack
                direction="row"
                spacing={1}
                sx={{
              ...
              >
                <Chip
                  icon={
                    <Typography
                      variant="caption"
                      sx={{ mr: -0.5, color: 'inherit', opacity: 0.7 }}
                    >
                      🔄
                    </Typography>
                  }
                  label={`${(fields.models_usage as any).prompt_tokens || '-'} Prompt`}
                  color="primary"
                  size="small"
                  variant="outlined"
                ...
                <Chip
                  ...
                  label={`${(fields.models_usage as any).completion_tokens || '-'} Completion`}
                  color="success"
                  size="small"
                  variant="outlined"
                  sx={{ height: '24px', '& .MuiChip-label': { px: 1, fontSize: '0.75rem' } }}
          ...
          {fields.source && (
            <Chip
              label={String(fields.source).replace(/['"]/g, '')}
              size="small"
              variant="outlined"
              sx={{
                mb: 1.5,
          ...
          <Box>{fields.content && <RenderData data={fields.content} level={1} />}</Box>
        </>
      </CardContent>
    </Card>
  );
...
function RenderData({ data, level = 0 }: RenderDataProps): JSX.Element {
  ...
  if (typeof data === 'string') {
            ...
            <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
              {fields.chat_message && (
                <Box sx={{ mb: 1 }}>
                  <Typography
                    variant="caption"
                    sx={{ fontWeight: 'bold', color: 'text.disabled', display: 'block', mb: 0.5 }}
                  >
                    Chat Message:
                  </Typography>
    ...
    if (reprMatch) {
      ...
      switch (typeName) {
        case 'TextMessage':
          return <TextMessageRenderer fields={fields} />;
          ...
          return (
            ...
            >
              <AccordionSummary
                ...
                sx={{
                  minHeight: '36px',
                  '.MuiAccordionSummary-content': { my: '8px', alignItems: 'center' },
                }}
              >
                <BuildOutlinedIcon fontSize="small" sx={{ mr: 1, color: 'secondary.main' }} />
                <Typography
                  variant="body2"
                  sx={{ fontWeight: 'bold', color: 'secondary.main', letterSpacing: 0.5 }}
                >
                  Tool Call Request
                </Typography>
                {fields.source && (
              ...
              <AccordionDetails sx={{ p: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                {fields.models_usage && (
                  <Box sx={{ mb: 1 }}>
                    <RenderData data={fields.models_usage} level={level + 1} />
                  </Box>
                )}
              ...
              <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                <Stack direction="row" spacing={1} alignItems="center" mb={1.5}>
                  <Chip
                    ...
                    label={String(fields.name).replace(/['"]/g, '') || 'Unknown Tool'}
                    color="secondary"
                    size="small"
                    sx={{ fontWeight: 'bold' }}
                  />
                  {fields.id && (
                    <Chip
                      label={`ID: ${String(fields.id).replace(/['"]/g, '')}`}
                      size="small"
                      variant="outlined"
                      sx={{ fontSize: '0.7rem' }}
                    />
                ...
                <Typography
                  variant="caption"
                  sx={{ fontWeight: 'bold', color: 'text.secondary', display: 'block', mb: 0.5 }}
                >
                  Arguments:
                </Typography>
    ...
    return (
      <Typography
        variant="body2"
        sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', lineHeight: 1.6, marginTop: '16px' }}
      >
        {data}
      </Typography>
    );
  ...
  if (
    typeof data === 'object' &&
    data !== null &&
    ('prompt_tokens' in data || 'completion_tokens' in data) &&
    !('content' in data)
  ) {
    const tokenData = data as { prompt_tokens?: number; completion_tokens?: number };
    return (
      <Stack
        direction="row"
        spacing={1}
      ...
      >
        <Chip
          icon={
            <Typography variant="caption" sx={{ mr: -0.5, color: 'inherit', opacity: 0.7 }}>
              🔄
            </Typography>
          }
          label={`${tokenData.prompt_tokens ?? '-'} Prompt`}
          color="primary"
          size="small"
          variant="outlined"
          sx={{ height: '24px', '& .MuiChip-label': { px: 1, fontSize: '0.75rem' } }}
        ...
        <Chip
          ...
          label={`${tokenData.completion_tokens ?? '-'} Completion`}
          color="success"
          size="small"
          variant="outlined"
          sx={{ height: '24px', '& .MuiChip-label': { px: 1, fontSize: '0.75rem' } }}
  ...
  return (
    <Box
      sx={{
        ...
        my: 1,
        p: 1,
        bgcolor: level > 0 ? 'action.focus' : undefined,
        borderRadius: 1,
      }}
    >
...
export const LogMessage: FC<LogMessageProps> = ({ data, type }) => {
