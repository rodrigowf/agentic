...
interface VoiceAssistantProps {
  nested?: boolean;
  onConversationUpdate?: () => void | Promise<void>;
}

function VoiceAssistant({ nested = false, onConversationUpdate }: VoiceAssistantProps): JSX.Element {
  ...
  const nestedWsRef = useRef<WebSocket | null>(null);
  const [sharedNestedWs, setSharedNestedWs] = useState<WebSocket | null>(null);
  const claudeCodeWsRef = useRef<WebSocket | null>(null);
  const [sharedClaudeCodeWs, setSharedClaudeCodeWs] = useState<WebSocket | null>(null);
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const streamRef = useRef<WebSocket | null>(null);
  const micStreamRef = useRef<MediaStream | null>(null);
  const eventsMapRef = useRef<Map<string | number, AgentMessage>>(new Map());
  const replayQueueRef = useRef<any[]>([]);
  const nestedScrollRef = useRef<HTMLDivElement | null>(null);

  // Track accumulating function-call arguments by call id
  const toolCallsRef = useRef<Record<string, any>>({});
  const lastVoiceToolCallRef = useRef<{ name: string | null; timestamp: number }>({ name: null, timestamp: 0 });
  const hasSpokenMidRef = useRef<boolean>(false);
  ...
  const postSdpOffer = async (mediaAddr: string, sessionId: string, clientSecret: string, sdp: string): Promise<string> => {
  ...
  const executeToolCall = async (callId: string, name: string, argsObj: any): Promise<{ ok: boolean; error?: string }> => {
    try {
