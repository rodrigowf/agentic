...
import { NestedAgentInsightsProps, AgentMessage } from '../../../types';
...
const NestedAgentInsights = ({
  ...
  truncateText,
  safeStringify,
}: NestedAgentInsightsProps): JSX.Element => {
  const nestedScrollRef = useRef<HTMLDivElement>(null);

  const nestedHighlights = useMemo<NestedHighlight[]>(() => {
    if (!messages || messages.length === 0) return [];
    const entries: NestedHighlight[] = [];
      ...
      switch (typeLower) {
        ...
        case 'tool_call_execution_event': {
          ...
          const hasError = firstContent?.is_error === true
            ...
            || (Array.isArray(contentItems) && contentItems.some((item: any) => item?.is_error || item?.status === 'error'));
