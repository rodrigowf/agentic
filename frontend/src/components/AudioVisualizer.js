 ...
 */
export default function AudioVisualizer({ stream, isActive = true, isMuted = false }) {
    ...
    if (!stream || !isActive || isMuted) {
      ...
      if (audioContextRef.current) {
        try {
          ...
          // Silently log to console, don't throw
          console.debug('AudioContext cleanup warning:', e.message);
        }
      ...
      return () => {
        ...
        if (audioContextRef.current) {
          try {
            ...
            // Silently log to console, don't throw
            console.debug('AudioContext cleanup warning on unmount:', e.message);
          }
        }
      };
    } catch (error) {
