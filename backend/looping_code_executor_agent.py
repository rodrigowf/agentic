...
class LoopingCodeExecutorAgent(CodeExecutorAgent):
    """
    A CodeExecutorAgent that overrides run_stream to loop its core logic
    (calling the LLM, handling code execution) until its own output contains "TERMINATE".
    """
        ...
        while True:
            ...
            try:
                async for evt in super().on_messages_stream(
                    messages=history, cancellation_token=cancellation_token
                ):
                    # Convert event to proper JSON format before yielding
                    if isinstance(evt, (TextMessage, ModelClientStreamingChunkEvent)):
                    ...
                    elif isinstance(evt, TextMessage) and evt.source == "code_executor":
                        current_iteration_new_history.append(evt)
                        accumulated_assistant_chunks = ""
                        last_assistant_text_message_content = None

            except asyncio.CancelledError:
                yield TextMessage(
                    content="[SYSTEM] Operation cancelled.", source="system"
                ).model_dump()
                raise
            ...
            if (
                accumulated_assistant_chunks
                and last_assistant_text_message_content is None
            ):
                chunk_message = TextMessage(
                    content=accumulated_assistant_chunks, source=self.name
                )
                if (
                    not current_iteration_new_history
                    or current_iteration_new_history[-1] != chunk_message
                ):
            ...
            if (
                final_content_this_iteration
                and "TERMINATE" in final_content_this_iteration
            ):
                break
